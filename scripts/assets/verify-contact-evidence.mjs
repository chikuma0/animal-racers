...
const history=await read(new URL('assets/source/western/upright-v2/history-resolution.json',root));
for(const [manifest,where] of [['frozen-cycle5.json',root],['diagnostic-v2-snapshot.json',base]]){
 const record=await read(new URL(manifest,base));for(const [path,expected] of Object.entries(record.files))assert.equal(hash(await fs.readFile(new URL(manifest==='frozen-cycle5.json'?(history.aliases[path]??path):path,where))),expected,path);
}
...
console.log(`Preserved Cycle5 controls, unchanged race/evidence and rejected diagnostic intact; 3 source/runtime pairs; 6 movies, ${pages} full-cycle pages, ${frames} paired frames. Canonical promotion is checked separately.`);
