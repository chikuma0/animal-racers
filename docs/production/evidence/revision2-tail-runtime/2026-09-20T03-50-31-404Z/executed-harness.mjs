...
await writeFile(resolve(directory,'executed-harness.mjs'),await readFile(new URL(import.meta.url)),{flag:'wx'});
...
async function still(name){
 ...
 await page.locator('#save').click();await page.waitForFunction(old=>document.querySelector('#status').value.startsWith('Saved ')&&document.querySelector('#status').value!==old,previous);
 ...
 assert.equal(receipt.settings.left,await page.locator('#left').inputValue());assert.equal(receipt.settings.action,await page.locator('#action').inputValue());
 assert.equal(receipt.settings.counter,await page.locator('#counter').isChecked());assert.ok(Math.abs(receipt.settings.elapsed-Number(await page.locator('#elapsed').inputValue()))<.00001);
 for(const [key,row] of Object.entries(handoff.records)){const [kind,form]=key.split('-');assert.equal(receipt.hashes[`assets/source/western/revision2/tail-repair/candidate/${kind}${form==='upright'?'-upright':''}.glb`],row.runtimeSha256);}
...
assert.deepEqual(errors,[]);assert.equal(records.length,27);assert.equal(new Set(records.filter(r=>r.kind==='pose-still').map(r=>r.receipt)).size,18);console.log('RUNTIME POSE CYCLES CAPTURED');
