...
def frozen():
 ...
 resolution=ROOT/'assets/source/western/upright-v2/history-resolution.json'
 aliases=json.loads(resolution.read_text())['aliases'] if resolution.exists() else {}
 for path,expected in data['files'].items():assert digest(ROOT/aliases.get(path,path))==expected,('Historical Cycle5 or unchanged race/evidence changed',path)
