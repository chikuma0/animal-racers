...
def display():build();views()
def audit():
 frozen();drivers=safe_load(OUT/'native-body-study.blend');r=json.loads((OUT/'display.json').read_text());body=bpy.data.objects['Mast2024-11'];rig=bpy.data.objects['ArmatureMast'];scene=bpy.context.scene
 assert signature(body)==r['bodyGeometryWeightsBefore'];assert not bpy.data.texts and not bpy.data.libraries;assert not drivers
 action=rig.animation_data.action;assert action.name=='Native shoulder strike crouch probe';assert list(action.frame_range)==[1.,73.]
 samples=[]
 for index in [0,6,18,30,36]:
  scene.frame_set(1+index*2);v,_=geometry(body);box=bounds(v);expected=r['probe']['samples'][index]['bounds'];error=max(abs(box[k][i]-expected[k][i]) for k in ['min','max'] for i in range(3));assert error<1e-4,(index,error)
  samples.append({'renderSample':index,'savedActionFrame':1+index*2,'bounds':box,'maxBoundsDifference':error})
 result={'sourceSha256':sha(OUT/'native-body-study.blend'),'recipeSha256':sha(Path(__file__)),'originalBodyGeometryWeightsSha256':signature(body),'savedAction':{'name':action.name,'frameRange':list(action.frame_range),'curves':len(action.fcurves)},'nativeConstraints':sum(len(b.constraints) for b in rig.pose.bones),'embeddedTexts':len(bpy.data.texts),'linkedLibraries':len(bpy.data.libraries),'drivers':drivers,'renderColorAttribute':body.data.color_attributes[body.data.color_attributes.render_color_index].name,'savedActionSamples':samples}
 (OUT/'audit.json').write_text(json.dumps(result,indent=2)+'\n');frozen();print('MAST_NATIVE_SAVED_SOURCE_AUDIT_OK',flush=True)
