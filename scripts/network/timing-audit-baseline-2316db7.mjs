...
const expectRematchRecovery = process.argv.includes('--expect-rematch-recovery');
...
function compile(node, counterfactual = false) {
  let code = node.getText(tree);
  if (counterfactual) {
    assert.equal((code.match(/data\.match\.phase === "countdown"/g) ?? []).length, 1);
    code = code.replace('data.match.phase === "countdown"', 'true /* harness-only counterfactual: accept authorized rematch epoch in any phase */');
  }
...
function makeEndpoint(api, clock, wire, role, options) {
  ...
  const receive = new Function('scope', `with(scope){${validators}\nreturn ${options.counterfactual && role === 'guest' ? compile(callbacks.receive, true) : compiledCallbacks.receive};}`)(s);
  net.onMessage = message => {
    const previous = s.lastSnapshot.current;
    receive(message);
    s.peakPendingEdges = Math.max(s.peakPendingEdges, ...Object.values(s.remoteReceiver.current.pending));
  ...
  s.setInput = patch => { Object.assign(s.input.current, patch); s.inputSender.current.update(s.input.current); };
  return s;
}
const profiles = [
...
async function scenario(profile, options = {}) {
  ...
  const collect = (s, slot) => {
    ...
    for (const receiver of [s.localReceiver.current, s.remoteReceiver.current]) metrics.peakPendingEdges = Math.max(metrics.peakPendingEdges, ...Object.values(receiver.pending));
    if (slot === 0 && m) {
      ...
      for (const event of m.events.filter(event => event.id > previousEvent)) {
        const stimulus = metrics.stimuli.find(item => item.epoch === host.epoch.current && item.slot === event.slot && item.key === event.type && item.authorityAt === null);
  ...
  const expectDeadlock = options.blackoutCountdown && !options.counterfactual && !expectRematchRecovery;
  await clock.until(() => finalAt !== null || (expectDeadlock && secondBeginAt !== null && clock.now > secondBeginAt + 10_000), start + 340_000);
  ...
  const result = {
    profile: profile.id, injected: profile, counterfactual: Boolean(options.counterfactual), blackoutAllRematchCountdownSnapshots: Boolean(options.blackoutCountdown),
  ...
  if (!expectDeadlock) assert.ok(sameResult && host.epoch.current === 'epoch-2', 'both complete rematch');
  else assert.ok(!sameResult && host.epoch.current === 'epoch-2' && guest.epoch.current === 'epoch-1' && host.ui.setInterrupted && !guest.ui.setInterrupted, 'consequential rematch deadlock reproduced');
  assert.ok(response.length === 6 && response.every(row => row.authorityAt !== null), 'all scripted short press intents reached authority');
  if (profile.inputBlackoutMs) assert.ok(response[0].inputToAuthorityMs >= profile.inputBlackoutMs, 'delayed cumulative edge reproduces stale action after input loss');
...
const pendingMaximum = Math.max(...Object.values(receiver.pending));
...
if (!expectRematchRecovery) results.push(await scenario(structuredClone(profiles[0]), { blackoutCountdown: true, counterfactual: true }));
const report = {
  auditedCommit: execFileSync('git', ['rev-parse', 'HEAD'], { cwd: root, encoding: 'utf8' }).trim(),
  expectedBehavior: expectRematchRecovery ? 'Real current-source rematch recovery required; no counterfactual callbacks.' : 'Frozen Cycle-6 rematch deadlock reproduction plus a hypothesis-only counterfactual; not production repair verification.',
  ...
  boundaries: ['Actual exported network, input-buffer, presentation and simulation code.', 'Actual AST-extracted app begin, animate, message-handler and rematch callbacks; only sockets, WebRTC primitive, clock, React refs/setters and renderer sink are modeled.', 'Counterfactual changes only an in-memory extracted guest condition. Frozen canonical files remain unchanged.', 'Near-zero values are synthetic. Slow fallback values are inspired by the historical small smoke sample, not a reconstruction or forecast.'],
  edgeBurst, results,
...
const artifactName = expectRematchRecovery ? './timing-audit-recovery-result.json' : './timing-audit-result.json';
