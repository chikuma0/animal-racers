...
const history=JSON.parse(await fs.readFile(new URL('assets/source/western/upright-v2/history-resolution.json',root),'utf8'));
for(const [path,expected] of Object.entries(frozen.files))assert.equal(hash(await fs.readFile(new URL(history.aliases[path]??path,root))),expected,path);
