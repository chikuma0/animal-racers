...
| Race/control feel | understandable touch, line choice, anticipation, jump and recovery | PARTIAL: Cycle10 repairs reproduced pointer-ownership failures; desktop buttons/keys and rematch work. Physical touch/feel UNVERIFIED | run simultaneous-finger and cancellation checks; measure obstacle sight/reaction distances |
...
| Delivery | playable link + editable sources/licenses/build docs + Japanese draft PR | PARTIAL: Japanese draft PR1, editable source and immutable f600be2 preview with scoped owner test access | complete required quality evidence before release; production remains prior version |
...
The isolated Lion hero study is rejected: three structural/provenance gates pass parent re-verification, while its visual/motion gate is abandoned for this approach because the repeated mane volumes remain below target. This does not abandon the required character-art outcome. Sources/rejected views/diagnosis are retained. The completed licensed Wolf assessment rejects direct adoption after three stills and all37 deformation samples; parent reverified its source/structure/evidence oracles. It is retained as anatomical reference only. Mast’s authored facial source is now being evaluated in both forms; its imported long curled mane is rejected. No research asset is promoted.

## Cycle9 — instrumented first-draw diagnosis

The recording-off solo baseline completed Lion/Wolf21.6/78.4 and yielded race/fight interval p95 of11ms, while first countdown still stalled870.2ms. New fixed-memory work histograms separate simulation, rendering and callback gaps. Most measured countdown work fell in draw submission, consistent with the installed Three source preparing shaders without drawing/uploading representative geometry during loading. This is a hypothesis supported by timing and source evidence, not a GPU trace.

The bounded loading-draw repair passes tests/lint/build and a fresh complete normal-control match. Nine loading draws take586.5ms; countdown draw max82.9ms versus the earlier320.9ms, while materially higher host load and changed viewport history prevent a clean causal comparison. Race/fight interval p95 of30/40ms remains a failure in the stressed sample. [Cycle9 evidence](evidence/CYCLE-9-PERFORMANCE.md) preserves the baseline, exact fingerprints, host contention and any comparison. The shared owner preview remains4ed5042. Phone, internet, all-roster motion and owner acceptance remain open.

## Cycle10 — independent physical press ownership

[Cycle10](evidence/CYCLE-10-CONTROLS.md) retains old-source negative controls for latched Jump after button rebinding, one pointer cancelling another held direction/guard, and keyboard input clearing touch. A bounded owner map and combined keyboard state repair these defects. Both host and guest clear old held intent on rematch. Five input tests, the actual callback oracle, all six network schedules and110 full-suite tests/lint/build pass at fingerprint bdbf3736f1bd4aa452750665aa500885c864546b4402df74fb06ef3595fbd94a.

Actual Wolf/Lion solo and rematch recordings reach13.6/86.4 and19.1/80.9. The first duel ended before the attempted browser input; that failure is explicit. The rematch registered Element/Strike/Guard clicks and12 rival HP loss, then returned correctly to selection. Both complete videos decode, and motion sheets were reviewed. This narrow regression does not pass physical multitouch, human control feel, all-matchup contact or enjoyment. Recording-on rematch race/fight p95 remains40/51ms on a loaded Mac. Required phone performance remains unverified.

The Mast head study is parent-verified as completed research only, with an explicit rework decision. Jaw clearance improves, but eyelids, neck topology, proportions and the retained mane/body prevent runtime adoption. Source/clip histories are preserved in local commit cab3151. A bounded full-body source assessment now tests whether preserving the authored continuous anatomy is a better construction path. Shared owner preview remains4ed5042; no local changes were silently published.

## Separate review build and completed source rejection

2026-09-17: deployed verified f600be234ec7f52527c0a4335c611d236ad39ea3 to its own immutable protected preview. Clean export, exact seven asset hashes, served revision and actual browser selection/lobby connection were checked; the scoped private link and updated phone protocol were delivered to the owner. Old4ed5042 branch alias and production remain unchanged. This is the current phone/input/network review build, not a release candidate. [Deployment evidence](evidence/review-f600be2-deployment.json) excludes private access tokens. Physical response remains pending.

The Mast native body assessment is complete and parent-verified as a rejection. Native connected skin avoids the cut-neck seam, but human fingers, feet, torso, overhead deformation and33,864 body triangles before mane require major reconstruction. All three full views and the complete37-frame probe are retained. No candidate entered runtime. The next bounded source review establishes access to a genuinely animal-specific editable base; topology, deformation and two-stance suitability must be inspected after lawful download. Main art and animation gates remain open.
