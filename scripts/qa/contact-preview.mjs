...
const files = ['src/championship/renderer.ts', 'src/championship/simulation.ts', 'src/championship/framing.ts', 'scripts/qa/contact-preview.ts'];
const hashes = {};
for (const file of files) hashes[file] = createHash('sha256').update(await readFile(resolve(root, file))).digest('hex');
for (const animal of ['lion', 'wolf', 'unicorn']) {
  const file = candidate ? resolve(candidate, `${animal}-upright.glb`) : resolve(publicRoot, `assets/western/${animal}-upright.glb`);
...
createServer(async (req, res) => {
  try {
    ...
    if (url.pathname.startsWith('/assets/')) {
      ...
      if (candidate && animal) file = resolve(candidate, `${animal}-upright.glb`);
