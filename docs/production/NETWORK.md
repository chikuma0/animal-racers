...
## Delivered repaired-roster replay — revision2-de401d12cd83

The source rules/transport/prediction bytes are unchanged from the full9-pair dab6 matrix. Six character GLBs and their provenance changed. Parent verified an explicit affected subset: Lion/Wolf plus full rematch, Wolf/Unicorn, Unicorn/Lion;4 complete accepted movies and8 role-rounds each with4 leaps,1 successful evade,1 actual counter and received damage. Both roles agree on all results. See browser-revision2/aggregate-2026-09-20T04-27-02-316Z.json. A Wolf/Unicorn Leap click timed out in the mixed run; the failed45second capture remains explicit, and a separate unchanged-build rerun passed. The subset oracle requires exactly the named pairs and rejects a missing required pairing. The default still requires all9. This remains shared-machine WebRTC functional/motion evidence, not separate-device internet or phone acceptance.
