...
def build():
 ...
 for form in ['race','upright']:
  ...
  base=Matrix.Translation(Vector((0,-.75,1.28)))@Matrix.Scale(2.6,4)@Matrix.Translation(-source_pivot)
  ...
  record={'form':form,'inputSha256':sha(OUT/'inputs'/f'lion-{form}.blend'),'bodySignatureBefore':before_body,'bodySignatureAfter':body_signature(body),'actionSignatureBefore':before_actions,'actionSignatureAfter':action_signature(),'removedHeadObjects':removed,'fitMatrix':[list(row) for row in fit],'facialBones':len(face.data.bones),'extractedHeadVertices':len(next(o for o in meshes if o.name.startswith('Mast authored')).data.vertices),'evaluatedMeshCosts':{o.name:evaluated(o) for o in bpy.context.scene.objects if o.type=='MESH'}}
  ...
  for time in [.18,.20,.21,.24,.28]:
   frame=1+time*30;bpy.context.scene.frame_set(int(frame),subframe=frame%1);point=probe_vertices(study_head);record['contactHeadForwardSamples'].append({'time':time,'forwardMax':max(-p.y for p in point),'bounds':bounds(point)})
  set_game_idle(form)
  record['selectedHeadSubdivision']=0;record['palette']='Authored gold/cream anatomy regions, dark nose/oral tissue, amber eyes';record['maneDecision']='Reject imported rolled curtain mane; retain unchanged canonical ruff as provisional control.';record['recipeSha256']=sha(Path(__file__))
...
def motion():
 frozen()
 for form in ['race','upright']:
  game,scene,cam=study_view(form,'candidate');face=bpy.data.objects['Mast isolated facial rig'];head=bpy.data.objects['Mast authored head and oral anatomy'];folder=OUT/'qa'/f'{form}-probe-frames';folder.mkdir(exist_ok=True);scene.render.resolution_x=384;scene.render.resolution_y=384;scene.render.fps=30
  ...
  probe_pose(face,0);rest=probe_vertices(head);edges=[tuple(e.vertices) for e in head.data.edges];lengths=[(rest[a]-rest[b]).length for a,b in edges];record=[]
  for i in range(31):
   t=i/15;probe_pose(face,t);v=probe_vertices(head);ratios=sorted((v[a]-v[b]).length/l for (a,b),l in zip(edges,lengths) if l>1e-7);assert all(math.isfinite(c) for p in v for c in p)
   record.append({'frame':i,'time':t,'bounds':bounds(v),'edgeRatioMin':ratios[0],'edgeRatioMax':ratios[-1],'edgeRatioP99':ratios[int(.99*(len(ratios)-1))],'edgesOver2x':sum(x>2 for x in ratios)})
