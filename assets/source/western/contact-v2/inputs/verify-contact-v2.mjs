...
const root=new URL('../../',import.meta.url),out=new URL('assets/source/western/contact-v2/',root),hash=b=>crypto.createHash('sha256').update(b).digest('hex');
const frozen=JSON.parse(await fs.readFile(new URL('frozen-cycle5.json',out),'utf8'));
...
for(const kind of ['lion','wolf','unicorn']){
 ...
 for(const variant of ['control','candidate']){
  const bytes=await fs.readFile(new URL(`${variant}/${kind}-upright.glb`,out));
