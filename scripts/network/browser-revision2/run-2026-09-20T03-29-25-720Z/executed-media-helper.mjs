...
const ownedRoot = fileURLToPath(new URL('./browser-revision2/', import.meta.url));
export async function prepareBrowserMedia(directory, files, onProgress = () => {}) {
  directory = resolve(directory);
  assert.ok(directory.startsWith(ownedRoot), 'only owned browser evidence directories');
  const processOptions = { timeout: 180_000, maxBuffer: 1024 * 1024 }, entries = [];
  const decode = async path => {
    const result = await processFile('ffmpeg', ['-v', 'error', '-nostdin', '-threads', '2', '-i', path, '-map', '0:v:0', '-map', '0:a?', '-fps_mode', 'passthrough', '-enc_time_base', '1:1000000', '-f', 'null', '-'], processOptions);
  ...
  for (const raw of files) {
    ...
    const entry = { rawFile: raw.slice(directory.length + 1), rawSha256, rawBytes, rawDecode: 'complete; no errors with passthrough microsecond output timebase', rawProbe,
      ...
      encoding: 'H264 CRF25,960px wide,30fps,AAC96k;2 threads. Constant-rate review copy, not original frame cadence or controlled performance evidence.' };
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  assert.ok(process.argv.length >= 4, 'Usage: node prepare-browser-media.mjs OWNED_RUN_DIRECTORY RAW_FILE [RAW_FILE...]');
  const directory = resolve(process.argv[2]);
  const entries = await prepareBrowserMedia(directory, process.argv.slice(3).map(p => resolve(p)), (kind, data) => console.log(kind, JSON.stringify(data)));
  await writeFile(resolve(directory, `media-recovery-${Date.now()}.json`), JSON.stringify(entries, null, 2) + '\n', { flag: 'wx' });
  console.log('PORTABLE BROWSER MEDIA VERIFIED');
}
