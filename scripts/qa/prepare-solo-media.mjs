...
const ownedRoot = fileURLToPath(new URL('../../docs/production/evidence/revision2-solo/', import.meta.url));
export async function prepareSoloMedia(directory, files, onProgress = () => {}) {
  directory = resolve(directory);
  assert.ok(directory.startsWith(ownedRoot) && directory !== ownedRoot, 'only owned solo evidence directories');
  ...
  for (const raw of files) {
    ...
    await processFile('ffmpeg', ['-v', 'error', '-nostdin', '-n', '-threads', '1', '-i', raw, '-map', '0:v:0', '-map', '0:a?', '-vf', 'scale=960:-2', '-r', '30', '-c:v', 'libx264', '-threads', '1', '-preset', 'veryfast', '-crf', '25', '-c:a', 'aac', '-b:a', '96k', '-movflags', '+faststart', destination], processOptions);
    ...
    const entry = { rawFile: raw.slice(directory.length + 1), rawSha256, rawBytes, rawDecode: 'complete; no errors with passthrough microsecond output timebase', rawProbe,
      ...
      encoding: 'H264 CRF25,960px wide,30fps,AAC96k;1 thread. Constant-rate review copy, not original frame cadence or controlled performance evidence.' };
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  assert.ok(process.argv.length >= 4, 'Usage: node prepare-solo-media.mjs OWNED_RUN_DIRECTORY RAW_FILE [RAW_FILE...]');
  const directory = resolve(process.argv[2]);
  const entries = await prepareSoloMedia(directory, process.argv.slice(3).map(p => resolve(p)), (kind, data) => console.log(kind, JSON.stringify(data)));
  await writeFile(resolve(directory, `media-recovery-${Date.now()}.json`), JSON.stringify(entries, null, 2) + '\n', { flag: 'wx' });
  console.log('PORTABLE SOLO MEDIA VERIFIED');
}
