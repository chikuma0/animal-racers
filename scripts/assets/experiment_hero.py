...
def mane(s):
 ...
 for row in range(rows):
  t=row/(rows-1)
  for j in range(cols):
   a=j*math.tau/cols;p=point(a,t);p.y-=.002+.23*math.sin(math.pi*t);pts.append(tuple(p))
 for row in range(rows-1):
 ...
 def furcolor(p):
  ...
  inner=SHADOW.lerp(RUSSET,.78);return inner.lerp(COPPER,.03+.11*rad+.035*crest)
 s.mesh('Hero connected radial cheek and jaw ruff',pts,faces,furcolor)
 # Individually placed cheek, jaw and dorsal streams. Each path changes
 # direction and overlaps the next mass; this is intentionally not an angular array.
 streams=[
 ([(.235,.09,.24),(.41,.16,.43),(.57,-.08,.33),(.50,-.35,.15)],.079,.28),
 ([(.245,.10,.15),(.43,.20,.32),(.66,-.06,.20),(.56,-.34,-.035)],.081,.32),
 ([(.258,.14,.035),(.49,.21,.16),(.64,-.15,.07),(.55,-.40,-.19)],.089,.22),
 ([(.245,.18,-.07),(.45,.25,-.01),(.62,-.04,-.18),(.48,-.32,-.37)],.085,.31),
 ([(.215,.21,-.19),(.39,.28,-.18),(.49,-.025,-.37),(.37,-.26,-.55)],.080,.28),
 ([(.14,.225,-.235),(.265,.25,-.34),(.32,.055,-.55),(.19,-.16,-.70)],.073,.25),
 ([(.34,-.045,.34),(.51,-.06,.49),(.61,-.35,.27),(.43,-.53,.10)],.086,.14),
 ([(.41,-.14,.12),(.58,-.16,.18),(.61,-.45,-.01),(.43,-.54,-.24)],.089,.12),
 ([(.35,-.17,-.19),(.55,-.12,-.24),(.54,-.34,-.45),(.32,-.46,-.59)],.082,.15),
 ]
 for sign in [-1,1]:
  for i,(points,w,light) in enumerate(streams):
   controls=[(sign*x,f,h) for x,f,h in points];s.curve('Hero authored sweeping ruff mass',controls,w,RUSSET.lerp(COPPER,light),steps=16,sides=8,flat=.66)
   if i in [0,3,5]:
    controls=[(sign*(x+.012),f+.016,h+.022) for x,f,h in points];s.curve('Hero restrained fire edge',controls,.016,COPPER.lerp(RUSSET,.24),steps=14,sides=5,flat=.60)
 for j in range(3):
  x=(j-1)*.062;c=[(x,.23,-.24),(x*1.2,.255,-.40),(x*.8,.04,-.61),(x*.6,-.15,-.72+.025*(j%2))]
  s.curve('Hero short throat mass',c,.055,RUSSET.lerp(COPPER,.19),steps=14,sides=6,flat=.64)
...
def build(form):
 ...
 # The smaller upright muzzle exposes the old collar/clasp at jaw height.
 # Attach this isolated costume to the chest below the jaw; body and rig stay exact.
 if form=='upright':
  for name in ['Leather neckerchief roll','Neckerchief folded point','Brass crest clasp','Raised elemental crest']:
   ob=bpy.data.objects[name]
   for v in ob.data.vertices:v.co.y-=.17;v.co.z-=.27
   if name=='Leather neckerchief roll':
    pts=[];faces=[]
    for i in range(28):
     a=i*math.tau/28;center=V((.425*math.sin(a),.04+.37*math.cos(a),1.645+.05*abs(math.sin(a))))
     outward=Vector((math.sin(a),-math.cos(a),0)).normalized()
     for j in range(6):
      b=j*math.tau/6;pts.append(tuple(center+outward*(.024*math.cos(b))+Vector((0,0,.026*math.sin(b)))))
    for i in range(28):
     for j in range(6):faces.append((i*6+j,i*6+(j+1)%6,((i+1)%28)*6+(j+1)%6,((i+1)%28)*6+j))
    materials=list(ob.data.materials);data=bpy.data.meshes.new('Hero fitted chest collar');data.from_pydata(pts,[],faces);data.update();ob.data=data
    for m in materials:data.materials.append(m)
    for poly in data.polygons:poly.use_smooth=True
   if name=='Neckerchief folded point':
    for v in ob.data.vertices:v.co.y=min(v.co.y,-.438+.28*v.co.x*v.co.x)
