...
| Animation | full cycles, contacts, transformation, varied species attacks | OPEN:14 racing/source clips plus11 dedicated upright clips per animal; Cycle5 actual rematch recorded; close-contact spacing and scene cut need repair | render/play clips, capture motion and fix foot/ground or joint artifacts |
...
| Scoring | bounded monotone contribution, recoverable split wins, robust edge cases | AUTOMATED PASS:35 simulation cases incl all9 matchups/seeds and adversarial pools; local result matched both clients | inspect actual tie/DNF celebration and all result presentation |
