...
import { prepareBrowserMedia } from './prepare-browser-media.mjs';
...
function verifyRound(host, guest, raceReports, expectedPair) {
  ...
  for (const row of actions) {
    ...
    assert.ok(row.counters >= 1, `slot${row.slot}: actual counter reply required inside the earned opening`);
...
function selfTest() {
  ...
  const lateReply = structuredClone(host); lateReply.playTrace.events = lateReply.playTrace.events.map(event => event.type === 'counter' ? { ...event, type: 'attack' } : event);
  assert.throws(() => verifyRound(lateReply, guest, [host, guest], ['lion', 'wolf']), /counter reply/);
...
for (let i = 0; i < args.length; i += 2) {
  assert.ok(['--url', '--manifest', '--revision', '--pairs', '--rematch', '--viewport', '--record'].includes(args[i]) && args[i + 1], 'Use --url URL --manifest PATH --revision LABEL [--pairs lion:wolf,...] [--rematch first|none] [--viewport 844x390] [--record both|host]');
...
const viewport = options.viewport ? options.viewport.split('x').map(Number) : [1100, 740];
assert.ok(viewport.length === 2 && viewport.every(v => Number.isInteger(v) && v >= 320 && v <= 1600), 'bounded WIDTHxHEIGHT viewport');
const recordingRoles = options.record ?? 'both';
assert.ok(['both', 'host'].includes(recordingRoles));
...
const mediaHelperBytes = await readFile(new URL('./prepare-browser-media.mjs', import.meta.url));
await writeFile(resolve(directory, 'executed-harness.mjs'), harnessBytes, { flag: 'wx' });
await writeFile(resolve(directory, 'executed-media-helper.mjs'), mediaHelperBytes, { flag: 'wx' });
await json(resolve(directory, 'harness-identity.json'), { harnessSha256: digest(harnessBytes), mediaHelperSha256: digest(mediaHelperBytes), arguments: args, note: 'Source snapshots of executed automation; not browser/app state.' });
...
async function startMeasurement(page, recording) {
  await panel(page, true);
  await page.getByRole('button', { name: 'Start fresh measurement', exact: true }).click();
  if (recording) await page.getByRole('button', { name: 'Record game canvas', exact: true }).click();
  await panel(page, false);
  if (recording) await page.locator('.recording-indicator').waitFor({ state: 'visible' });
}
...
async function race(page, role) {
  ...
  while (Date.now() < end) {
    ...
    for (const at of [16, 40, 61, 88]) if (progress >= at && !acted.has(`jump${at}`)) {
      acted.add(`jump${at}`); await tap(page, role, 'Leap');
    }
    const steering = [[10, 'ArrowRight'], [20, 'ArrowRight'], [35, 'ArrowLeft'], [45, 'ArrowLeft'], [58, 'ArrowRight'], [72, 'ArrowRight'], [85, 'ArrowLeft']];
...
async function duel(pages, prefix) {
  ...
  await delay(4500); // Preserve the complete score reveal and trophy entrance.
}

async function launch(role, pairName) {
  const profile = resolve(directory, `profile-${pairName}-${role}`);
  const context = await chromium.launchPersistentContext(profile, { channel: 'chrome', headless: true, viewport: { width: viewport[0], height: viewport[1] }, deviceScaleFactor: 1, acceptDownloads: true });
...
try {
  for (const [index, pair] of pairs.entries()) {
    ...
    try {
      ...
      for (let round = 1; round <= (index === 0 && rematchFirst ? 2 : 1); round++) {
        ...
        await Promise.all(pages.map((page, slot) => startMeasurement(page, recordingRoles === 'both' || slot === 0)));
  ...
  const summary = { ...plan, startedAt: new Date(started).toISOString(), completedAt: new Date().toISOString(), url: options.url, sourceFingerprint: manifest.sourceFingerprint, buildId: buildId.trim(), expectedReportRevision: options.revision,
    requestedPairs: pairs, rematchFirst, viewport, recordingRoles, results, served: Object.fromEntries(served), servedValidationErrors: networkErrors, media, timeline, fatalFailure,
