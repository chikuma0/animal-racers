...
def mane(s):
 # Connected, directionally fluted fur volume. Long grooves travel with the
 # surface; the silhouette clumps emerge from it rather than floating as bars.
 profiles=[(-.62,.095,.075,-.19),(-.43,.29,.17,-.20),(-.17,.42,.25,-.225),(.10,.45,.27,-.215),(.33,.43,.28,-.19),(.49,.33,.26,-.145),(.585,.19,.17,-.14),(.63,.035,.035,-.16)]
 def profile(t):
  u=t*(len(profiles)-1);i=min(len(profiles)-2,int(u));a=u-i
  p0=Vector(profiles[max(0,i-1)]);p1=Vector(profiles[i]);p2=Vector(profiles[i+1]);p3=Vector(profiles[min(len(profiles)-1,i+2)])
  return .5*((2*p1)+(-p0+p2)*a+(2*p0-5*p1+4*p2-p3)*a*a+(-p0+3*p1-3*p2+p3)*a*a*a)
 pts=[];faces=[];rows=38;cols=80
 for row in range(rows):
  t=row/(rows-1);h,rx,rf,center=profile(t)
  for j in range(cols):
   a=j*math.tau/cols;flow=a+.28*math.sin(h*4)+.22*h;ridge=math.cos(20*flow+.45*math.sin(flow*5));flute=.016*ridge+.008*math.cos(39*flow+.7);edge=math.sin(math.pi*t)**.6
   x=math.sin(a)*(rx+flute*edge);f=center+math.cos(a)*(rf+flute*edge);z=h+.009*math.sin(flow*17)*edge
   pts.append((x,f,z))
 ...
 def furcolor(p):
  x,f,h=p;a=math.atan2(x,(f+.20)*1.6);flow=a+.28*math.sin(h*4)+.22*h;crest=(1+math.cos(20*flow+.45*math.sin(flow*5)))/2
  return RUSSET.lerp(COPPER,.08+.46*crest).lerp(SHADOW,.23*(1-math.cos(a))/2)
 s.mesh('Hero connected flowing mane volume',pts,faces,furcolor)
 def lock(name,c,w,color):s.curve(name,c,w,color,steps=10,sides=6,flat=.55)
 for j in range(7):
  x=(j-3)*.064;z=.35+.045*abs(j-3)/3
  c=[(x,.085,z),(x+.05,.125,.49),(x+.07,-.02,.675+.015*math.sin(j*2)),(x+.035,-.20,.60+.025*math.sin(j*2))]
  lock('Hero flame crown crest',c,.048,COPPER.lerp(RUSSET,.18+.09*(j%3)))
 for sign in [-1,1]:
  for tier in range(3):
   for j in range(3):
    x=.275+.032*j;h=.33-tier*.17+.02*math.sin(j);f=.055-j*.03
    endh=-.17-tier*.20-.035*math.sin(j*2+tier)
    c=[(sign*x,f,h),(sign*(.44+.02*j),.075-j*.02,h-.04),(sign*(.50+.02*math.sin(j)),.018-.04*j,endh+.14),(sign*(.445+.04*math.sin(j+tier)),-.10-.04*j,endh)]
    lock('Hero cheek flame stream',c,.052+.005*math.cos(j+tier),COPPER.lerp(RUSSET,.22+tier*.10))
 for j in range(7):
  x=(j-3)*.064;c=[(x,.075,-.16),(x*1.4,.12,-.30),(x*1.35,.085,-.48),(x*.8,-.01,-.65+.04*abs(j-3)/3)]
  lock('Hero throat flame stream',c,.046,COPPER.lerp(RUSSET,.38+.04*abs(j-3)))
